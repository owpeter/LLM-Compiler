...
基于MCTS算法与LLM对GEMM算子进行调优

## 定义算子workload

首先需要定义GEMM算子的workload，即算子的输入输出张量的大小和数据类型。

## 初始化

随机从`LLM-Compiler/InfiniCore/scripts/profile/gemm.yaml`中读取GEMM算子的schedule参数作为初始算子，记为$p_0$，将$p_0$作为MCTS的初始状态。
