...
基于MCTS算法与LLM对rms_norm算子进行调优

## 定义算子workload

首先需要定义rms_norm算子的workload，即算子的输入输出张量的大小和数据类型。

## 初始化

随机从`LLM-Compiler/InfiniCore/scripts/profile/rms_norm/rms_norm.yaml`中读取rms_norm算子的schedule参数作为初始算子，记为$p_0$，将$p_0$作为MCTS的初始状态。
...
根据新的状态 $p_{i+1}$，调用`LLM-Compiler/InfiniOpt/gemm_xgboost/gemm_xgboost.py`中的`predict_xgboost`函数对 $p_{i+1}$ 进行预测，得到新的算子的性能分数。
