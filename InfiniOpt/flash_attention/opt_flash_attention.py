...
# Add xgboost folder to path to import predict_xgboost
current_dir = os.path.dirname(os.path.abspath(__file__))
flash_xgboost_dir = os.path.join(current_dir, "xgboost")
sys.path.append(flash_xgboost_dir)

try:
    from flash_attention_xgboost import predict_xgboost
except ImportError:
    print(f"Error: Could not import predict_xgboost from {flash_xgboost_dir}")
    sys.exit(1)

# Constants
FLASH_ATTN_YAML_PATH = "/root/LLM-Compiler/InfiniCore/scripts/profile/flash_attention/flash_attention.yaml"
MODEL_PATH = os.path.join(flash_xgboost_dir, "xgboost_model.json")
META_PATH = os.path.join(flash_xgboost_dir, "xgboost_model_meta.json")
...
def _pick_task_from_workload_cfg(workload_cfg: Dict) -> str:
    if "prefill" in workload_cfg:
        return "prefill"
    keys = list(workload_cfg.keys())
    if not keys:
        raise ValueError("No workload task found in workload config")
    return keys[0]


def get_default_workload(workload_cfg: Dict) -> Dict:
    task = WORK_LOAD.get("task")
    if task not in workload_cfg:
        task = _pick_task_from_workload_cfg(workload_cfg)

    task_cfg = workload_cfg[task]
    workload = {"task": task}
    for key, values in task_cfg.items():
        if key in WORK_LOAD:
            workload[key] = WORK_LOAD[key]
        elif isinstance(values, list) and values:
            workload[key] = values[0]
        else:
            workload[key] = values
    return workload
...
def predict(schedule: Dict, workload: Dict) -> float:
    # Build the exact feature set required by flash_attention_xgboost metadata.
    features = {
        "batch": workload["batch"],
        "num_heads": workload["num_heads"],
        "q_len": workload["q_len"],
        "total_kv_len": workload["total_kv_len"],
        "head_dim": workload["head_dim"],
        "block_m": schedule.get("block_m", 64),
        "block_n": schedule.get("block_n", 64),
...
def format_flash_attention_workload(workload: Dict) -> str:
    parts = [
        f"task={workload.get('task')}",
        f"batch={workload.get('batch')}",
        f"num_heads={workload.get('num_heads')}",
        f"q_len={workload.get('q_len')}",
        f"kv_len_buffer={workload.get('kv_len_buffer')}",
        f"total_kv_len={workload.get('total_kv_len')}",
        f"head_dim={workload.get('head_dim')}",
        f"is_causal={workload.get('is_causal')}",
    ]
    return ", ".join(parts)
...
def construct_prompt(node: Node, schedule_space: Dict) -> str:
    """
    Construct the prompt for the LLM based on opt_flash_attention.md requirements.
    """
    history = []
    curr = node
    while curr:
    ...
    example_json = {}
    for k, values in schedule_space.items():
        if isinstance(values, list) and values:
            example_json[k] = values[0]
        else:
            example_json[k] = values

    prompt = f"""
你是一名算子调优专家，现在需要根据以下信息，对flash-attention算子进行调优。
Current Hardware: {HARDWARE_NAME}
Workload: {format_flash_attention_workload(node.workload)}
Search Space: {json.dumps(schedule_space, ensure_ascii=False)}

History of optimizations (from initial to current):
"""
    for i, item in enumerate(history):
        prompt += f"Step {i}: Config={json.dumps(item['schedule'], ensure_ascii=False)}, Score={item['performance']}\n"
...
def run_mcts(iterations: int, verbose: bool = True, output_path: Optional[str] = None):
    ...
    schedule_space, workload_cfg = load_config(FLASH_ATTN_YAML_PATH)

    # 2. Define workload
    workload = get_default_workload(workload_cfg)
    ...
    print(f"Best Schedule: {json.dumps(best_node.schedule, ensure_ascii=False, indent=2)}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Optimize flash-attention operator using MCTS and LLM.")
    parser.add_argument("--iterations", type=int, default=40, help="Number of MCTS iterations")
    parser.add_argument("--verbose", action="store_true", help="Print verbose output")
    parser.add_argument(
        "--output",
        type=str,
        default=os.path.join(current_dir, "best_flash_attention_result.json"),
