...
def build(
    ...
    max_workers_env = os.getenv("NINETOOTHED_MAX_WORKERS", "")
    max_workers = int(max_workers_env) if max_workers_env.isdigit() else 0

    with concurrent.futures.ProcessPoolExecutor(max_workers=max_workers or None) as executor:
        futures = []
